...
function Math() {
    ...
    <div style={{padding: 50}}>
        ...
        <p>
            ...
            <ol>
                ...
                <li>
                    ...
                    position at <MathInline formula = "t+1"/>. This is a consequence of the symmetry in the recursion of the Leapfrog scheme.
        ...
        <p>
            ...
            <ul>
                <li>2 bodies in the simulation came too close together. Since acceleration is proportional 
                    to <MathInline formula = "\frac{1}{\|\vec{r}_i - \vec{r}_j\|^3}"/>, if bodies get too close together, 
            ...
            the step size, <MathInline formula = "h"/>. This controls the accuracy of the integrator, 
        ...
        <p>
            ...
            If we compute this adaptive time step at each time <MathInline formula = "t"/>, we will be able to improve the performance of the integrator.
            A simple example of this is given by:
            <MathBlock formula = "h^*(t) = c \times \underset{1 \leq i < j \leq n}{\min}\left\{\frac{\Delta x_{ij}(t)}{\Delta v_{ij}(t)}\right\}"/>
            where at time <MathInline formula = "t"/>:
            <ul>
                <li>
                    <MathInline formula = "c"/> is a proportionality constant used to modulate the size of the time step
                </li>
                <li>
                    <MathInline formula = "\Delta x_{ij}(t) = \|\vec{r}_i(t) - \vec{r}_j(t)\|"/>, the distance between 2 bodies
                </li>
                <li>
                    <MathInline formula = "\Delta v_{ij}(t) = \|\vec{v}_i(t) - \vec{v}_j(t)\|"/>, the magnitude of the difference 
        ...
        <p>
            ...
            <MathBlock formula = "h(t) = \frac{h^*(t) + h^*(t+1)}{2}"/>
            With this, we get all the ebenfits of a time-symmetric integrator, whilst being able to adapt 
            our integration based on how the system is behaving. Admittedly, computing <MathInline formula = "h(t)"/> is quite 
            involved since to perform a single integration step we need to:
            <ol>
                <li>
                    Compute <MathInline formula = "h^*(t)"/> in the current system.
                </li>
                <li>
                    Perform a (false) integration step using <MathInline formula = "h^*(t)"/>.
                </li>
                <li>
                    From the resulting system, we can compute <MathInline formula = "h^*(t+1)"/>.
                </li>
                <li>
                    Compute <MathInline formula = "h(t) = \frac{h^*(t) + h^*(t+1)}{2}"/>.
                </li>
                <li>
                    Use <MathInline formula = "h(t)"/> to perform the actual integration step in the current system.
                </li>
            </ol>
            However, this is well worth it, since you obtain a powerful integration scheme, which can easily integrate 
            most initial conditions, provided you modulate <MathInline formula = "c"/> appropriately.
        </p>
    </div>
    );
}
