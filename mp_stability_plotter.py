...
def get_stability_cell(row, col, perturb, n_trials, collision_tolerance, escape_tolerance, steps=10 ** 4, delta=10 ** -2,
                         tolerance=10 ** -2, adaptive_constant=0.1, delta_lim=10 ** -5):

    print(f"{row},{col},{os.getpid()}")
...
def mp_get_stability_matrix(perturb, n_trials, collision_tolerance, escape_tolerance, steps=10 ** 4, delta=10 ** -2,
                         tolerance=10 ** -2, adaptive_constant=0.1, delta_lim=10 ** -5):
    """
    ...
    for i in range(n):
        for j in range(n):
            args.append((i, j, perturb, n_trials, collision_tolerance, escape_tolerance, steps, delta,
                         tolerance, adaptive_constant, delta_lim))
    ...
    with Pool(n_cpus) as pool:
        results = pool.starmap(func = get_stability_cell, iterable = args)
...
def mp_plot_stability_matrix(perturb, n_trials, collision_tolerance, escape_tolerance, n_ticks=10, steps=10 ** 4,
                          delta=10 ** -2, tolerance=10 ** -2, adaptive_constant=0.1, delta_lim=10 ** -5, **kwargs):
    """
    ...
    stability_matrix = mp_get_stability_matrix(perturb, n_trials, collision_tolerance, escape_tolerance, steps, delta,
                                            tolerance, adaptive_constant, delta_lim)
    ...
    cax = ax.imshow(stability_matrix,
                    extent=(-n - perturb * 0.5, n + perturb * 0.5, -n - perturb * 0.5, n + perturb * 0.5),
